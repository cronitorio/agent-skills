...
2. Check `cronitor auth login --help` for PKCE and callback instructions. Version 33.6's device-code login does not work with Cronitor's external sign-in integration. If the installed version displays a user code, use existing API-key authentication until a PKCE release is installed. Explain that login installs a persistent machine credential that allows unattended access with the displayed permissions.
3. When the browser and CLI run on the same computer, use `cronitor auth login`. On a remote host, have the human run `cronitor auth login --no-browser` in an interactive terminal they can access directly. They open the printed authorization URL, sign in or create an account, and paste the full resulting localhost callback URL into that terminal prompt, even if the browser shows a connection error. Passwords and Google, GitHub, or required SAML authentication stay in the browser. Never ask for the callback URL in chat or accept it through a tool argument that records it in the conversation. If the human cannot access the prompt, report that blocker and use an already-authorized connection instead.
4. Keep the login process running until the local callback or pasted URL completes it. If it expires or is denied, report that result; don't repeatedly start new login attempts without the human being ready.
