...
For interactive CLI setup, check `cronitor auth login --help` for PKCE support. Use `cronitor auth login` locally. For a remote host, the human runs `cronitor auth login --no-browser` in their terminal, signs in or creates an account through the displayed browser URL, then pastes the full callback URL directly into the waiting hidden CLI prompt. Never request the callback URL in chat. `cronitor signup` is an alias. Check `cronitor auth status`, then make a read-only resource call. Use the same config path throughout; see `recipes.md#cli-signup-and-login`. Do not log out automatically: `cronitor auth logout` revokes the machine credential and jobs using it stop authenticating.
